...
void Quaternion::createFromEuler(float pitch, float yaw, float roll, Quaternion* dst)
{
    M_ASSERT(dst);
...
void Quaternion::computeEuler(float* pitch, float* yaw, float* roll)
{
...
void Quaternion::lerp(const Quaternion& q1, const Quaternion& q2, float t, Quaternion* dst)
{
    ...
    dst->x = sclp * q1.x + sclq * end.x;
    dst->y = sclp * q1.y + sclq * end.y;
    dst->z = sclp * q1.z + sclq * end.z;
    dst->w = sclp * q1.w + sclq * end.w;
}
...
void Quaternion::fromToRotation(const Vector3& fromDirection, const Vector3& toDirection, Quaternion * dst) {
	//assert(fromDirection.sqrMagnitude() > 0.0f && toDirection.sqrMagnitude() > 0.0f);
	Vector3 unitFrom = fromDirection;
	unitFrom.normalize();
	Vector3 unitTo = toDirection;
	unitTo.normalize();
	const float d = unitFrom.dot(unitTo);
	...
	else if (d <= -1.0f)
	{
		...
		Vector3 axis;
		Vector3::cross(unitFrom, Vector3(1, 0, 0), &axis);
		if (axis.length() < 1e-6)
		{
			...
			Vector3::cross(unitFrom, Vector3(0, 1, 0), &axis);
		}
		...
		Quaternion::createFromAxisAngle(axis, 3.14f, dst);

	}
	else
	{
		// Scalar component.
		const float s = sqrt(unitFrom.length() * unitTo.length())
			+ unitFrom.dot(unitTo);

		// Vector component.
		Vector3 v;
		Vector3::cross(unitFrom, unitTo, &v);

		// Return the normalized quaternion rotation.
		dst->set(v.x, v.y, v.z, s);
		dst->normalize();
