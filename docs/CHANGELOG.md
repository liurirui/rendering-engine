...
## 未提交 - 修正渲染架构与资源管理中的过时注释

- 改动类型：文档 / 注释维护
- 建议提交标题：`docs: 修正渲染架构与资源管理中的过时注释`

### 改动摘要

本轮只修正注释和架构文档，不改变运行时行为。重点同步 RenderGraph 依赖排序、EditorUI 职责、AssetManager 路径缓存状态、Example 输入层和当前 OpenGL 配置，删除已经移除的 NaN 旧实现注释。

### 具体改动

- 将 RenderGraph “顺序 Pass 容器”的过时说明改为“提交收集 + 资源依赖分析 + 稳定拓扑排序”。
- 明确 EditorUI 可以触发资源加载和 Shader 重载，但不拥有资源或渲染对象生命周期。
- 说明 AssetManager 当前路径统一尚未达到 canonical/大小写归一化级别。
- 删除 Example 中引用不存在 `io` 变量的 ImGui 模板注释，并明确输入层由应用侧维护鼠标捕获。
- 修正 OpenGL 4.2/GLSL 330 当前配置的说明，删除 MathUtil 已移除实现的旧注释。
- 同步 `ARCHITECTURE.md` 的 RenderGraph 描述，避免文档与代码实现冲突。

### 验证情况

- 项目源码和文档 UTF-8 检查通过。
- 过时注释关键字检索通过；第三方目录中的原始注释未修改。
- `git diff --check` 通过。
